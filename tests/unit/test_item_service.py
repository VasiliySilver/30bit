...
async def test_filter_items_by_kind(item_service, test_user):
    ...
    await item_service.create_item(test_user.id, ItemCreate(
        title="Book 1", kind=ItemKind.BOOK, status=ItemStatus.PLANNED,
        priority=Priority.HIGH, notes=None, tag_ids=[]
    ))
    await item_service.create_item(test_user.id, ItemCreate(
        title="Article 1", kind=ItemKind.ARTICLE, status=ItemStatus.READING,
        priority=Priority.NORMAL, notes=None, tag_ids=[]
    ))
...
async def test_filter_items_by_status(item_service, test_user):
    ...
    reading = await item_service.get_user_items(
        test_user.id, status=ItemStatus.READING
    )
